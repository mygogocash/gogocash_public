    ...
    <Form.Field name={name} className="relative" asChild>
      <Switch.Root
        ...
        className={`relative h-[25px] w-[42px] cursor-default rounded-full ${
          isChecked ? 'bg-[var(--primary-4)]' : 'bg-[white]'
        } shadow-[0_2px_10px] shadow-[var(--primary-4)] outline-none focus:shadow-[0_0_0_1px] focus:shadow-[var(--primary-4)] data-[state=checked]:bg-[var(--primary-4)]`}
        id="airplane-mode"
      >
        <Switch.Thumb
          className={`block size-[21px] translate-x-0.5 rounded-full  ${
            isChecked
              ? 'bg-white border border-[var(--primary-4)]'
              : ' bg-[var(--primary-4)]'
