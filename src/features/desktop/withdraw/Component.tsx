...
import useWithdrawWeb3 from '@/hooks/useWithdrawWeb3';
...
const Component = () => {
  ...
  const {
    ...
    mutate,
    error,
  } = useSWR<ResponseWithdrawCheck>(data, fetcherPost, {
  ...
  const handleWithdraw = async () => {
    try {
      ...
      if (session?.user._id && dataConversion) {
        // if (id == idT) {
        ...
        if (account) {
          ...
          await withdrawCashback({
            ...
            userAddress: account,
            totalCashbackAmount: dataConversion.netAmount?.toString(), // 10 tokens with 6 decimals
            conversionIdHashes: dataConversion.data.map((item) =>
          ...
          <div className="flex items-center justify-between">
            ...
            <div>
                  ...
                  mutate()
                    .then(() => {
                      if (error) {
                        ...
                        if (!dataConversion || dataConversion.netAmount === 0) {
              ...
              >
                ...
                {account
                  ? chainId !==
                    Number(process.env.NEXT_PUBLIC_CHAIN_ID_WITHDRAW)
                    ? 'Switch Network'
                    : 'Withdraw'
                  : 'Connect Wallet'}
              </button>
